...
- **Mono vs sans — five-voice taxonomy** (refined 2026-05-07): Geist Mono is the "machine voice" (codes, IDs, **data-tier numerics** in tables/badges/IDs, eyebrow labels). Geist Sans is the "human voice" (titles, body, button labels, **hero summary numerics ≥24px**). The hero-numeric tier (`<HeroNumeric>` primitive at `src/components/ui/hero-numeric.tsx`) splits the operational mono register from a presentation register: KPI rail values (CompactKpi `default` size = text-2xl/8 = 24px), full-page hero metrics (CMP-013 `8,241` at lg = text-3xl/9 = 32px), and panel heroes (Top Keys `$1,147.82`) all render as `font-sans tabular-nums font-medium` — `tabular-nums` carries the cell-padding mono affordance while sans signals "presented summary." **Below ~20px, numerics revert to mono regardless of role** — modal KpiTile at text-lg, table cells, badge contents stay mono. Roughly 60/40 mono/sans on operational surfaces.
...
- **`HeroNumeric` primitive** (codified 2026-05-07): single source of truth for sans-hero numerics across the app. Two sizes — `default` (text-2xl/8 = 24px, used by `CompactKpi` value + Top Keys hero) and `lg` (text-3xl/9 = 32px, used by full-page hero metrics like CMP-013's `8,241`). Recipe: `font-sans font-medium tabular-nums tracking-tight text-ink-900`. **Don't hand-roll this recipe** — every hero summary numeric should consume the primitive so the typographic tier stays single-sourced. If a new use case needs a third size, add a variant to the primitive instead of inlining classes.
- **`Badge` primitive defaults to `font-mono tabular-nums`** (codified 2026-05-07). Status codes (`200`, `500`), counters, and any numeric content inside `<Badge>` render mono automatically. Word-label badges ("clean", "info") also render mono — appropriate for the operator-tool register. Per-instance `font-mono` overrides are now redundant; new specimens shouldn't add them.
- **`SelectTrigger` asymmetric padding** (codified 2026-05-07): every size variant in `select.tsx` uses `pl-N pr-(N-1)` — 12/8 at `xs`/`sm`, 16/12 at `default`/`lg`. Optical balance rule: any trigger or button with a leading or trailing icon has more padding on the text side because letterforms have crisp edges that want air, while chevrons/arrows have built-in whitespace inside their bounding box. Mirrors the `Button` primitive's `data-icon` idiom. Don't symmetrize SelectTrigger padding without acknowledging the visual imbalance it reintroduces.
- **`TablePaginationFooter` primitive** (codified 2026-05-07): single source of truth for table pagination chrome at `src/components/ui/table-pagination-footer.tsx`. Composes count summary + rows-per-page select + windowed page links. Used by CMP-011 sortable, CMP-013 requests, CMP-014 conversations. State (page + rowsPerPage) stays in the parent; the primitive is controlled. **Don't hand-roll pagination footers** — extend the primitive (new prop, new variant) if behavior diverges. The `buildPageWindow` helper is exported for callers that need the truncation pattern outside the footer.
- **Filter-pill toolbar pattern** (codified 2026-05-07): `<SelectTrigger size="sm">` filter pills in dense table toolbars render with **chevron only — no leading category icon**. Generic filter glyphs (funnel, key, eye) are decorative noise when 2-3+ filters share the same toolbar — the chevron already signals "this opens" and the label does the disambiguation. Exception: dropdowns where the leading icon carries category-specific information that aids pre-label scanning, AND where it's used consistently across 4+ filters in the same surface. CMP-013 requests + CMP-014 conversations follow the no-icon rule.
...
- **Five-voice typographic taxonomy** — locked 2026-05-07. Hero summary numerics ≥24px = sans tabular via `<HeroNumeric>`; data numerics (table cells, IDs, badge contents, modal sub-tier) = mono. Don't revert hero numerics to mono without acknowledging the tier split. Don't extend `HeroNumeric` below the 20px boundary.
- **`Badge` primitive font-mono default** — locked 2026-05-07. `font-mono tabular-nums` lives in the base CVA classes. Don't strip it to bring sans into the badge family — every status/code/counter use case is mono and the consistency is load-bearing.
- **`SelectTrigger` asymmetric padding rule** — locked 2026-05-07. `pl-N pr-(N-1)` across all four sizes. Don't symmetrize without re-litigating the optical-balance discussion (see chevron-bounding-box reasoning).
- **`HeroNumeric` and `TablePaginationFooter` primitives** — locked 2026-05-07 as the single source of truth for their respective patterns. Don't re-inline the recipes in artboards; extend the primitive instead (new size variant, new prop). The "no custom on composed pages" rule depends on these being the canonical home.
- **No leading category icon on filter-pill SelectTriggers in dense toolbars** — locked 2026-05-07 (CMP-013, CMP-014). Generic filter glyphs are visual noise next to the chevron-down. Don't reintroduce them unless the toolbar grows past 3 filters AND the icons carry category-specific information used consistently across all of them.
