...
- [x] **16. Reorganize modal — surface message text** _(2026-05-06)_
  ...
  Done: Restructured the row drill-in end-to-end. (1) **Surface change**: `<Dialog>` → new `<Sheet>` primitive at `src/components/ui/sheet.tsx` — right-docked drawer is the category convention (Datadog / Honeycomb / Sentry / Grafana / Vercel) and preserves table context in the user's peripheral vision. Built on the same `@base-ui/react/dialog` Base UI primitive as Dialog so focus management, ESC, scroll-lock, and ARIA wiring come for free. tw-animate keyframes (slide-in-from-right + matching backdrop fade at 300ms) — earlier mix of CSS transition + tw-animate was reading choppy. (2) **Tab count 4 → 3**: Security folded into Audit since both are compliance-flavored (runtime guardrail checks + cryptographic anchor proof); section eyebrows distinguish the two halves within the Audit tab. (3) **Default tab now Messages** so the prompt/response is visible on first open — the PM's primary ask. (4) **KPI rail (Latency / Cost / Tokens In / Out) lifted out of the former Summary tab into a persistent block above the tabs**, single bordered container with `before:` hairline dividers between tiles, matching CMP-012's KPI rail pattern. (5) **Message bubble fills removed** (bg-ink-100 / bg-blue-50 → border-only) — outline preserves the per-message segmentation gestalt without the chat-app weight. (6) **Body scrolls** via `flex-1 overflow-y-auto` wrapper around the tabs — header + KpiRail + divider stay pinned at top, footer pinned at bottom, long Audit content (5 guardrail checks + 6 anchor rows) reachable without losing persistent header context. Footer continues to swap actions by active tab — Audit shows Copy Proof + View on DE; other tabs show Copy ID + Open Conversation. Shipped as two commits: `5e6880d` (Sheet primitive) + `de0ea3a` (CMP-013 conversion + restructure).
